...
void test01() {
    ...
    cout << arr1[2] << endl; // 未初始化默认？
    int arr2[5] = {0, 1}; //定义并部分初始化
    cout << arr2[4] << endl; // 未初始化默认？
    int arr3[5] = {0, 1, 2, 3, 4}; //定义并初始化了数组array
    int arr4[] = {0, 1, 2, 3, 4}; // b和a的效果是一样
    const int len = 5;
    int arr5[len]; // C++矩阵维度可以为变量
    int arr6[len] = {0, 1, 2, 3, 4}; // C++矩阵维度可以为变量
    ...
    delete[] array2;
    const int len3 = 100; // C++矩阵维度可以为变量
    int *array3 = new int[len3]; //分配了长度为100的数组array
    delete[] array3;
    ...
    //动态二维数组
    // 1.指针数组
    int(*n2Arr0)[2] = new int[3][2];
    delete[] n2Arr0;
    int M1 = 3; // C++矩阵维度可以为变量
    int(*n2Arr1)[2] = new int[M1][2]{{1, 1}, {2, 2}, {3, 3}}; // 列必须为常量
    for (int i = 0; i < 3; i++) {
        for (int j = 0; j < 2; j++) {
            cout << "n2Arr1[" << i << "][" << j << "]: ";
            cout << n2Arr1[i][j] << endl;
        }
    }
    delete[] n2Arr1;
