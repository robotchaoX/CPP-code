...
void test07(int argNum = 5, const int argNum_c = 6) {
    // 全局const受到只读保护，修改报错?？？
    cout << "m_A = " << m_A << endl;
    cout << "&m_A = " << &m_A << endl;
    int *pa = (int *)&m_A; // 临时指针
    ...
    *p = 200; // 修改临时开辟的内存,指针无法修改原局部const,不报错
    ...
    int Var = 30;
    const int c_b = Var; // const=变量，会分配内存,分配内存的都可以用指针修改
    cout << "c_b = " << c_b << endl; // 30
    int *cp = (int *)&c_b;
    *cp = 333; // 通过指针修改了
    cout << "c_b = " << c_b << endl; // 333
    Var = 444; // const与初始化变量无关??
    cout << "c_b = " << c_b << endl; // 333
    ...
    // 形参可以初始化数组长度
    int arrArg[argNum] = {1, 2, 3, 4}; // 可以初始化数组
    cout << "arrArg[1] = " << arrArg[1] << endl;
    // const形参可以初始化数组长度
    int arrArg_c[argNum_c] = {1, 2, 3, 4}; // 可以初始化数组
    cout << "arrArg_c[1] = " << arrArg_c[1] << endl;
}
